...
export class ShopifyService {
  ...
  async getAllOrders(limit = 50) {
    try {
      ...
      const rawData = (response.body as any).orders ?? [];
  ...
  async getOrderById(orderId: string) {
    try {
      ...
      const rawData = (response.body as any).order ?? null;
  ...
  async fulfillOrder(orderId: string, trackingNumber: string, trackingCompany: string) {
    try {
      ...
      const fulfillmentOrders = (foResponse.body as any).fulfillment_orders ?? [];

      if (fulfillmentOrders.length === 0) {
        throw new BadRequestException(`No fulfillment orders found for order ${orderId}`);
      }
      ...
      if (!fulfillmentOrder) {
        throw new BadRequestException(`No open fulfillment orders found for order ${orderId}`);
      }
      ...
      return {
        success: true,
        data: (fulfillResponse.body as any).fulfillment,
      };
  ...
  async updateShipment(fulfillmentId: string, trackingNumber: string, trackingCompany: string) {
    try {
      ...
      return {
        success: true,
        data: (response.body as any).fulfillment,
      };
